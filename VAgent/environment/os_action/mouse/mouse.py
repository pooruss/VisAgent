...
class Mouse:
    ...
    def move(self, **kwargs):
        ...
        if text != None:
            screenshot = self.computer.state.screenshot
            ...
            if len(coordinates) == 0:
                ...
                # print(coordinates)
                x, y = coordinates[0]["coordinates"][0]
                x *= self.computer.display.width
                y *= self.computer.display.height
                x = int(x)
                y = int(y)
    ...
    def click(self, button="left", clicks=1, interval=0.1, click_type = "single", **kwargs):
        if ("text" in kwargs) | ("x" in kwargs and "y" in kwargs) | ("icon" in kwargs):
            self.move(**kwargs)
        if click_type == "single":
            pyautogui.click(button=button, clicks=clicks, interval=float(interval))
        elif click_type == "double":
            pyautogui.doubleClick(button=button, interval=float(interval))
        elif click_type == "triple":
            pyautogui.tripleClick(button=button, interval=float(interval))
