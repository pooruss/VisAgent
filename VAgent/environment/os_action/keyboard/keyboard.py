...
class Keyboard:
    ...
    def press(self, keys="", delimiter="/", presses=1, interval=0.1):
        if len(delimiter) != 0:
        ...
        """
        time.sleep(0.15)
        pyautogui.press(keys, presses=presses, interval=float(interval))
        time.sleep(0.15)

    def hotkey(self, keys, delimiter, interval=0.1):
        """
        ...
        """
        print(keys, delimiter)
        args = keys.split(delimiter)
        time.sleep(0.15)

        modifiers = ["command", "option", "alt", "ctrl", "shift", "cmd"]
        if "darwin" in platform.system().lower() and len(args) == 2:
            ...
            if keystroke.lower() == "cmd":
                keystroke = "command"
            
            if modifier.lower() == "cmd":
                modifier = "command"
            ...
            pyautogui.hotkey(*args, interval=float(interval))
        time.sleep(0.15)
